import os, sys, time, urllib.request, json, random, string
from seleniumbase import SB

# ==========================================
# 💡 核心配置 (G4F.GG 双重核武制导版 - 三服务器循环版)
# ==========================================
TARGET_URLS = [
    "https://g4f.gg/rena",
    "https://g4f.gg/deku"
...
def generate_username():
    """生成：1个随机大写字母 + 4个小写字母，如 Xbmt"""
    first = random.choice(string.ascii_uppercase)
    rest = ''.join(random.choice(string.ascii_lowercase) for _ in range(4))
    return first + rest
...
for TARGET_URL in TARGET_URLS:
    MC_USERNAME = generate_username()

    print(f"\n===== 🚀 开始执行极速续期 (终极双重核武狙击版) =====")
    print(f"🎯 目标: {TARGET_URL}")
    print(f"👤 随机用户名: {MC_USERNAME}")
    ...
    with SB(uc=True, proxy=proxy_str, headless=False, window_size="1920,1080") as sb:
        try:
            ...
            # 截图文件名带上服务器名，避免覆盖
            server_name = TARGET_URL.split("/")[-1]
            sb.save_screenshot(f"screenshots/{server_name}_1_page_loaded.png")

            print(f"✍️ 尝试填入游戏ID: {MC_USERNAME}")
            try:
            ...
            print("⏳ 射击完毕！静默等待 8 秒，让子弹飞一会儿...")
            time.sleep(8)

            try:
                sb.save_screenshot(f"screenshots/{server_name}_2_result.png")
                print("📸 最终战况截图已保存。")
            except:
                print("⚠️ 截图保存失败。")

            print(f"✅ [{server_name}] 流程执行完毕！")
            send_tg(f"✅ 服务器 [{server_name}] 续期脚本运行完毕！\n随机用户名: {MC_USERNAME}\n【破盾方式: 双重核武物理盲狙】请查阅 GitHub 截图确认战果。")

        except Exception as e:
            print(f"❌ 发生致命错误: {e}")
            try:
                os.makedirs("screenshots", exist_ok=True)
                sb.save_screenshot(f"screenshots/{server_name}_error.png")
            except:
                pass
            send_tg(f"❌ [{server_name}] 自动续期崩溃: {e}")
